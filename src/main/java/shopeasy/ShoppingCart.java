...
public class ShoppingCart {
    ...
    public void addItem(Product product, int quantity) {
        // Pre-conditions: Product exists and quantity is valid
        assert product != null : "Product cannot be null";
        assert quantity > 0 : "Quantity must be > 0";
        
        int prevItemCount = itemCount(); // Keep track of item count before modifying

        for (CartItem item : items) {
            if (item.getProduct().getId().equals(product.getId())) {
                item.setQuantity(item.getQuantity() + quantity);
                
                // Post-conditions: Item count didn't drop and total is not negative
                assert itemCount() >= prevItemCount : "Item count should not decrease";
    ...
    public double applyDiscount(double discountRate) {
        // Pre-condition: Discount is a valid percentage
        ...
        // Post-condition: Discounted price makes sense compared to original
        assert discountRate == 0 || discounted <= rawTotal : "Discounted total must be <= raw total";
